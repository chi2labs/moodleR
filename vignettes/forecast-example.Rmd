...
A seasonal pattern is obvious, and clearly the activity mostly follows a weekly pattern. We can use either of the subsets of data to create an arima model, available in the `stats` package. We set the order and period parameters for a vanilla prediction.

```{r}
my_model_2 <- arima(my_timeseries_2$Users,
...
And we can combine the two in one plot

```{r}
my_timeseries_2 %>%
  select(date,Users) %>% 
  mutate(Type = "Observed") %>% 
  rbind(predicted_users_2 %>% mutate(Type = "Predicted")) -> combined_ts
my_plot <- combined_ts %>% 
